...
export default function NewsFeedScreen({
  ...
  return <View style={styles.fill}>
    ...
    <Animated.FlatList
      data={items}
      keyExtractor={(item: NewsFeedItem) => item.id}
      ...
      ListHeaderComponent={<View>
        <PageHead title="News Radar" onBack={onBack} actions={<PageActions />} />
        <View style={styles.controls}>
        <View style={styles.controlRow}>
          <MastheadMeta size={11}>{totalMatching === totalAvailable ? `${totalAvailable} STORIES` : `${totalMatching} OF ${totalAvailable} STORIES`}</MastheadMeta>
          <View style={[styles.segment, { borderColor: t.rule }]}>{SOURCES.map((option) => <Pressable
        ...
        <FlatList horizontal data={topics} keyExtractor={(item) => item.value} showsHorizontalScrollIndicator={false}
          contentContainerStyle={styles.topicRow} renderItem={({ item }) => <Pressable
            ...
            style={({ pressed }) => [styles.topic, { borderColor: item.value === topic ? t.surfaceAnchor : t.rule, backgroundColor: item.value === topic ? t.surfaceAnchor : t.surfacePaper }, pressed ? { opacity: 0.7 } : null]}
      ...
      renderItem={({ item }: { item: NewsFeedItem }) => <StoryCard item={item} onOpen={() => onOpen(item)} />}
      ItemSeparatorComponent={() => <View style={styles.gap} />}
      ListEmptyComponent={!refreshing ? <View style={[styles.empty, { borderColor: t.rule }]}><Tray size={22} color={t.inkFaint} /><Text style={{ color: t.inkStrong, fontFamily: sans(600) }}>No stories match these filters</Text></View> : null}
...
function StoryCard({ item, onOpen }: { item: NewsFeedItem; onOpen: () => void }) {
  ...
  return <Pressable onPress={onOpen} style={({ pressed }) => [styles.card, { borderColor: t.rule, backgroundColor: pressed ? t.surfaceSoft : t.surfacePaper }]}>
    {imageUrl ? <Image source={imageUrl} style={styles.image} contentFit="cover" cachePolicy="memory-disk" recyclingKey={item.id} transition={160} accessibilityIgnoresInvertColors /> : <View style={[styles.image, { backgroundColor: t.surfaceSoft }]} />}
    <View style={styles.cardBody}>
      <View style={styles.badges}><Text style={[styles.badge, { borderColor: t.rule, color: t.inkMuted }]}>{item.kind === 'gpfa' ? item.articleType : item.topic}</Text>{item.kind === 'gpfa' && item.isMemberOnly ? <LockSimple size={13} color={t.inkMuted} /> : null}</View>
...
const styles = StyleSheet.create({
  ...
  topicRow: { gap: 8 }, topic: { flexDirection: 'row', gap: 6, borderWidth: 1, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }, count: { fontFamily: mono(400), fontSize: 11 },
  gap: { height: 14 }, card: { borderWidth: 1, borderRadius: 12, overflow: 'hidden' }, image: { width: '100%', aspectRatio: 16 / 8 }, cardBody: { padding: 15, gap: 8 },
  ...
  title: { fontFamily: sans(600), fontSize: 19, lineHeight: 25.5 }, preview: { fontFamily: sans(400), fontSize: 15, lineHeight: 22.5 },
