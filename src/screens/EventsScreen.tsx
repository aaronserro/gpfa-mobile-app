...
export default function EventsScreen({
  ...
  const head = (
    <PageHead title="Events" onBack={onBack} backLabel="Back to More" actions={<PageActions />}>
        <View style={[styles.viewSegment, { borderColor: t.rule }]}>
  ...
  return (
    <View style={[styles.fill, { backgroundColor: t.surfacePage }]}>
      ...
      {viewMode === 'calendar' ? (
        <Animated.ScrollView showsVerticalScrollIndicator={false} {...handlers}>
          {head}
        ...
        >
          ...
          <View style={styles.introRow}>
            ...
            <MastheadMeta size={11}>{`${visible.length} EVENT${visible.length === 1 ? '' : 'S'}`}</MastheadMeta>
          </View>

          {visible.length ? (
            <View style={[styles.eventBand, { backgroundColor: t.surfacePaper, borderColor: t.rule }]}>
                ...
                >
                  ...
                  <View style={styles.eventBody}>
                    <View style={styles.metaLine}>
                      ...
                      <MastheadMeta size={10.5}>{event.timeLabel}</MastheadMeta>
                    </View>
    ...
    <View style={[styles.fill, { backgroundColor: t.surfacePage }]}>
      ...
      >
        ...
        <View style={styles.detailLead}>
          <View style={styles.metaLine}>
            <Text style={[styles.eventType, { color: t.brandAmber }]}>{event.type}</Text>
            <MastheadMeta size={11}>{event.dateLabel.toUpperCase()}</MastheadMeta>
          </View>
        ...
        <View style={[styles.factCard, { backgroundColor: t.surfacePaper, borderColor: t.rule }]}>
        ...
        <View style={[styles.attendeeCard, { backgroundColor: t.surfacePaper, borderColor: t.rule }]}>
          <EventAttendees
        ...
        <View style={[styles.agendaCard, { backgroundColor: t.surfacePaper, borderColor: t.rule }]}>
          {event.agenda.map((item, index) => (
            <View key={`${item.time}-${item.title}`} style={[styles.agendaRow, index > 0 && { borderTopWidth: 1, borderTopColor: t.rule }]}>
      ...
      {event.status === 'upcoming' && event.registrationOpen && (
        <View style={[styles.actionBar, { backgroundColor: t.surfacePaper, borderTopColor: t.rule, paddingBottom: Math.max(insets.bottom, 12) }]}>
          <View style={styles.eventBody}>
...
function DateBlock({ month, day }: { month: string; day: string }) {
  const { t } = useTheme();
  return (
    <View style={[styles.dateBlock, { backgroundColor: t.surfaceSoft, borderColor: t.rule }]}>
...
function Fact({
  ...
  return (
    <View style={[styles.factRow, divided && { borderTopWidth: 1, borderTopColor: t.rule }]}>
...
const styles = StyleSheet.create({
  fill: { flex: 1 },
  viewSegment: { flexDirection: 'row', gap: 3, marginBottom: 8, padding: 3, borderWidth: 1, borderRadius: 12 },
  viewButton: { flex: 1, minHeight: 31, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  viewLabel: { fontFamily: sans(600), fontSize: 12.5 },
  segment: { flexDirection: 'row', gap: 3, padding: 3, borderRadius: 12 },
  segmentButton: { flex: 1, minHeight: 34, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  segmentLabel: { fontFamily: sans(600), fontSize: 12.5 },
  ...
  introTitle: { fontFamily: sans(600), fontSize: 19, letterSpacing: trackDisplay(19) },
  introCopy: { marginTop: 3, fontFamily: sans(400), fontSize: 13.5 },
  ...
  eventType: { fontFamily: sans(600), fontSize: 11.5 },
  eventTitle: { fontFamily: sans(600), fontSize: 15, lineHeight: 20.5 },
  eventLocation: { marginTop: 3, fontFamily: sans(400), fontSize: 13, lineHeight: 18.5 },
  badgeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 8 },
  empty: { marginHorizontal: 20, borderWidth: 1, borderRadius: 12, alignItems: 'center', padding: 28 },
  emptyTitle: { marginTop: 10, fontFamily: sans(600), fontSize: 16 },
  emptyCopy: { marginTop: 4, textAlign: 'center', fontFamily: sans(400), fontSize: 13.5, lineHeight: 19.5 },
  ...
  detailSummary: { marginTop: 12, fontFamily: sans(400), fontSize: 15, lineHeight: 23.5 },
  factCard: { borderWidth: 1, borderRadius: 12 },
  factRow: { flexDirection: 'row', gap: 12, alignItems: 'center', padding: 14 },
  factLabel: { fontFamily: sans(600), fontSize: 15 },
  factDetail: { marginTop: 2, fontFamily: sans(400), fontSize: 13 },
  attendeeCard: { borderWidth: 1, borderRadius: 12, padding: 14 },
  sectionHeading: { marginTop: 24, marginBottom: 10, fontFamily: sans(600), fontSize: 18, letterSpacing: trackDisplay(18) },
  agendaCard: { borderWidth: 1, borderRadius: 12 },
  agendaRow: { flexDirection: 'row', gap: 14, padding: 14 },
  agendaTime: { width: 62, fontFamily: mono(600), fontSize: 11.5 },
  agendaTitle: { fontFamily: sans(600), fontSize: 15, lineHeight: 20 },
  agendaDetail: { marginTop: 3, fontFamily: sans(400), fontSize: 13, lineHeight: 18.5 },
  ...
  secondaryActionText: { fontFamily: sans(600), fontSize: 14.5 },
  ...
  actionTitle: { fontFamily: sans(600), fontSize: 14.5 },
  actionMeta: { marginTop: 2, fontFamily: sans(400), fontSize: 12.5 },
  primaryAction: { minHeight: 44, borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 16 },
  primaryActionText: { fontFamily: sans(600), fontSize: 14.5 },
});
