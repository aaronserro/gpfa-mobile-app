...
export default function PollQuestionnaire({
  ...
  return (
    <View style={[styles.panel, { borderColor: t.rule, backgroundColor: t.surfacePaper }]}>
      <View style={styles.progressRow}>
        <Text style={[styles.progress, { color: t.inkMuted }]}>
          Question {currentIndex + 1} of {poll.questions.length}
        </Text>
        <MastheadMeta size={10.5}>{poll.closes}</MastheadMeta>
      </View>
      ...
      <View style={styles.actions}>
        <Pressable
          ...
          style={[styles.secondary, { borderColor: t.rule }]}
        >
...
const styles = StyleSheet.create({
  panel: { gap: 12, marginTop: 14, borderWidth: 1, borderRadius: 12, padding: 14 },
  progressRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 },
  progress: { fontFamily: sans(600), fontSize: 12.5 },
  question: { fontFamily: sans(600), fontSize: 16, lineHeight: 22.5 },
  options: { gap: 8 },
  ...
  optionLabel: { flex: 1, fontFamily: sans(500), fontSize: 14.5, lineHeight: 20 },
  result: { fontFamily: sans(600), fontSize: 12.5 },
  ...
  responded: { fontFamily: sans(500), fontSize: 12.5 },
});
